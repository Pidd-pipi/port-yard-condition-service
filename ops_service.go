...
func (s *OpsService) Transition(ctx context.Context, id string, expected int, target OpsStatus, actor string) (OpsRecord, error) {
	...
	if err := s.state.Move(updated.Status, target, "operator update"); err != nil {
		return OpsRecord{}, err
	}
...
// Update replaces the mutable fields of an existing record under optimistic
// concurrency control; expected is the revision the caller last observed.
func (s *OpsService) Update(ctx context.Context, id string, expected int, record OpsRecord) (OpsRecord, error) {
	record.ID = id
	record = normalizeOpsRecord(record)
	if err := s.policy.Check(record); err != nil {
		return OpsRecord{}, err
...
func (s *OpsService) Delete(ctx context.Context, id string) error {
	if err := s.store.Delete(ctx, id); err != nil {
		return err
	}
	s.audit.Add(id, "deleted", "operator")
	return nil
}

// RecentEvents returns the most recent audit events across all records.
func (s *OpsService) RecentEvents(limit int) []OpsEvent {
	if limit < 1 {
		limit = 50
	}
